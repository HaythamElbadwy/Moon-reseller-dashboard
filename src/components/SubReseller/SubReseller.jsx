...
export default function SubReseller() {
  ...
  const addNewSubReseller = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`https://moon-smy9.onrender.com/reseller/addSubReseller`, {
        method: 'POST',
        headers: {
  ...
  const getSubReseller = async () => {
    ...
    try {
      const response = await fetch(`https://moon-smy9.onrender.com/reseller/getSubReseller`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `moonOM0${localStorage.getItem('authToken')}`
