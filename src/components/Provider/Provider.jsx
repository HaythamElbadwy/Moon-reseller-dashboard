...
export default function Provider() {
  ...
  const getProviderInfo = async () => {
    ...
    try {
      const response = await fetch(`https://moon-smy9.onrender.com/reseller/info`, {
        method: 'GET',
        headers: {
  ...
  const getProvider = async (page) => {
    ...
    try {
      const response = await fetch(`https://moon-smy9.onrender.com/reseller/getByCode?page=${page}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `moonOM0${localStorage.getItem('authToken')}`
  ...
  const addGenerateCode = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`https://moon-smy9.onrender.com/reseller/generateCode`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `moonOM0${localStorage.getItem('authToken')}`
  ...
  const getSearch = async (macAddress) => {
    ...
    try {
      const response = await fetch(`https://moon-smy9.onrender.com/reseller/searchByMac?macAddress=${macAddress}&type=provider`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `moonOM0${localStorage.getItem('authToken')}`
        ...
        <div className={`${styles.provider_options} mt-12 flex items-center justify-around`}>
          ...
          <button type="button"
           ...
           border-[#B5A5C9] hover:bg-[#B5A5C9] focus:ring-4 focus:outline-none
            focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:border-[#B5A5C9] dark:text-[#B5A5C9] dark:hover:text-white
             dark:hover:bg-[#B5A5C9] dark:focus:ring-[#B5A5C9]">
